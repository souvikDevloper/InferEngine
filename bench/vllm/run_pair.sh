...
start_inferengine() {
  INFERENGINE_BACKEND="${INFERENGINE_BACKEND:-transformers}" \
    ...
    INFERENGINE_VLLM_PORT="${INFERENGINE_VLLM_PORT:-8002}" \
    INFERENGINE_VLLM_LOG="${INFERENGINE_VLLM_LOG:-$RESULT_DIR/inferengine-vllm-backend.log}" \
    INFERENGINE_MAX_MODEL_LEN="$MAX_MODEL_LEN" \
    INFERENGINE_TENSOR_PARALLEL_SIZE="$TENSOR_PARALLEL_SIZE" \
    INFERENGINE_VLLM_GPU_MEMORY_UTILIZATION="$VLLM_GPU_MEMORY_UTILIZATION" \
